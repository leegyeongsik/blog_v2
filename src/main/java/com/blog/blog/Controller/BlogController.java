...
public class BlogController {
    ...
    public BlogResponseDto updatePost(@PathVariable Long id , @RequestBody BlogRequestDto blogRequestDto){
        BlogResponseDto blogResponseDto = new BlogResponseDto(blogService.updatePost(id,blogRequestDto)); // 커밋이 완료된상태라 blog 주소의 modifiedAt의 값이 변경되었음
        return blogResponseDto;                                                                                    // -> 다시 그 주소를 blogResponseDto 에 넣어서 출력을해줌
    }
    @DeleteMapping("/blogs/{id}")
    public String deletePost(@PathVariable Long id , @RequestBody BlogRequestDto blogRequestDto ){
